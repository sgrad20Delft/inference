...
class Preprocessor:
    def __init__(self, model_type, model_name, user_preprocess_fn=None, input_size=(224, 224),layout="NCHW"):
        """
    ...
    def preprocess(self, image_path):
        ...
        if self.user_preprocess_fn:
            return self.user_preprocess_fn(image_path, self.layout)
        if self.user_preprocess_fn and isinstance(self.user_preprocess_fn, dict):
            # dictionary like {"pytorch": fn1, "huggingface": fn2}
            self.custom_per_model = self.user_preprocess_fn
        else:
            self.custom_per_model = None
            self.user_preprocess_fn = self.user_preprocess_fn
